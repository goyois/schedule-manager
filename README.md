...
- **카테고리 관리** - 일정 분류를 위한 카테고리 CRUD. ADMIN이 만든 카테고리는 모든 유저에게 보이는 "기본 설정 카테고리"로 취급되어 일반 USER는 수정·삭제할 수 없지만, ADMIN 본인은 삭제할 수 있습니다(단, 해당 카테고리를 쓰는 일정이 남아있으면 ADMIN도 삭제 불가). 소유자가 없는 레거시 카테고리는 누구나 수정·삭제할 수 있고, USER가 만든 카테고리는 본인에게만 보입니다. 앱 기동 시 `일상`/`업무`/`자기계발`/`식단`/`운동` 5개는 ADMIN 계정이 하나라도 있으면 그 계정 소유의 기본 설정 카테고리로 자동 생성됩니다(이미 같은 이름이 있으면 건너뜀).
...
- **AI 일정 추천 챗봇** - Claude(`ChatClient`)와 구조화된 응답 기반으로 지속 대화합니다. 매 턴 현재 날짜/시각을 프롬프트에 함께 실어 보내 "오늘"/"내일"/"이번 주" 같은 상대적 표현을 모델이 임의로 추측하지 않고 정확히 계산하게 합니다. 모델이 매 답변을 `SCHEDULE_RECOMMENDATION`(새 일정 추천)/`SCHEDULE_UPDATE`(기존 일정 수정 제안)/`MANDALART_FILL`(만다라트 채우기)/`GENERAL`(그 외 일반 답변) 중 하나로 분류해서, 그에 맞는 UI만 보여줍니다. 새 일정 추천은 "이번 주에 운동, 팀 회의, 병원 예약 등록해줘"처럼 한 번에 여러 개를 요청해도 각각 독립된 카드로 제안되며(최대 10개), 카드마다 "수동 등록"(폼 검토 후 저장) / "자동 등록"(즉시 저장, "AI 추천 자동 등록" 설정이 켜져 있으면 버튼 클릭 없이 응답 즉시 자동 반영)을 따로 선택할 수 있고, 여러 개일 땐 "전체 자동 등록"으로 한 번에 반영할 수도 있습니다. 기존 일정 수정 제안은 "수정"(수정 폼을 열어 검토 후 저장) / "수정 반영"(추가 창 없이 즉시 반영) 중 선택할 수 있습니다. "OO 만다라트 채워줘"처럼 요청하면 대상 보드를 찾아 그 자리에서 바로 채우고 결과만 보여줍니다(빈 칸만 채우므로 검토 단계 없이 자동 적용). ADMIN을 제외한 일반 유저는 Redis 기반으로 분당 5회까지만 요청할 수 있습니다.
