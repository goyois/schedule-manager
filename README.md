...
- **카테고리 관리** - 일정 분류를 위한 카테고리 CRUD. ADMIN이 만든 카테고리는 모든 유저에게 보이는 "기본 설정 카테고리"로 취급되어 일반 USER는 수정·삭제할 수 없지만, ADMIN 본인은 삭제할 수 있습니다(단, 해당 카테고리를 쓰는 일정이 남아있으면 ADMIN도 삭제 불가). 소유자가 없는 레거시 카테고리는 누구나 수정·삭제할 수 있고, USER가 만든 카테고리는 본인에게만 보입니다.
