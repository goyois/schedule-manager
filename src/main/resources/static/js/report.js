...
const TREND_DRAW_MS = 3900; // 선 하나가 그려지는 데 걸리는 시간(CSS의 report-trend-line 애니메이션 시간과 맞춰야 함)
const TREND_SERIES_STAGGER_MS = 480; // 카테고리(시리즈)마다 그려지기 시작하는 시점을 얼마씩 늦출지
...
async function loadStats() {
  ...
  try {
    ...
    animateNumber(totalCountEl, stats.totalCount, 3900, (v) => `${Math.round(v)}건`);
    animateNumber(completionRateEl, stats.completionRate * 100, 3900, (v) => `${Math.round(v * 10) / 10}%`);
